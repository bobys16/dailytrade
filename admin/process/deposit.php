...
$body = file_get_contents('../dashboard/mail/depo_status.html');
...
if($isRouter){
	...
	if($type == 'cancel'){
		...
		if(mysqli_num_rows($cek) < 1){
			...
			if($update){
				$result['mail'] = Elz_Mail($user_f['email'], $user_f['full_name'], "#".$t_id." Your Top Up Rejected!", str_replace(array('{{NAME}}','{{STATUS}}'),array($user['full_name'],'CANCELED'),$body));
				$result['success'] = 'true';
		...
		if(mysqli_num_rows($cek) < 1){
			...
			if($update){
				$result['mail'] = Elz_Mail($user_f['email'], $user_f['full_name'], "#".$t_id." Your Top Up Has Been Succesfully Processed!", str_replace(array('{{NAME}}','{{STATUS}}'),array($user['full_name'],'SUCCESS'),$body));
				$cur = $idr['fund'] + $f_c['amount'];
				mysqli_query($connect, "UPDATE idr SET fund=fund+".$f_c['amount']." WHERE uid='".$f_c['userid']."'");
				$result['success'] = 'true';
				$result['msg'] = 'Succesfully change the status !';
			}else{
				$result['msg'] = 'There is some trouble proccessing request!';
