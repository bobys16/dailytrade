...
if($http == "true") {
    ...
    $get_routes = mysqli_query($connect, "SELECT * FROM admin_routes");
    $route = array();
    ...
    if(array_key_exists($path, $route)) {
        if($route[$path]['is_auth'] == 1 && isAuthenticated($_SESSION)) {
            $user = getUserEntireData($_SESSION['admin_id']);
...
function isAuthenticated($session) {
    ...
    if(isset($session['login'])) {
        ...
        $query = mysqli_query($connect, "SELECT * FROM admin WHERE id='".$uid."'");
